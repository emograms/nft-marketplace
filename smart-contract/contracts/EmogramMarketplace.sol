...
contract EmogramMarketplace is AccessControl, ReentrancyGuard {
  ...
  modifier isAllowedToFinishAuction(address _account, uint256 _auctionId) {
    require(
      (emogramsOnAuction[_auctionId].highestBidder == _account) ||
        (emogramsOnAuction[_auctionId].seller == _account),
      "Not allowed to finish auction"
    );
    _;
  }
  ...
  modifier isInitialAuctionPeriod() {
    require(
      isInitialAuction == true,
      "The initial auction period has already ended"
    );
    _;
  }
  ...
  // Check if the item exists in emogramsOnSale array
  ...
  modifier itemExistsAuction(uint256 id) {
    require(
      id <= emogramsOnAuction.length && emogramsOnAuction[id].auctionId == id,
      "Could not find item"
    );
    _;
  }
  ...
  // Add new founders
  function addFounder(address _newFounder) public onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  {
    require(
      msg.value >= emogramsOnSale[id].price,
    ...
    onlyRole(FOUNDER_ROLE)
  {
    ...
    if (emogramsOnAuction.length == 3) {
      for (uint256 i = 0; i < 3; i++) {
        if (emogramsOnAuction[i].highestBidder == msg.sender) {
          endAuctionWithNoBid(
            _tokenAddress,
            emogramsOnAuction[i].tokenId,
            emogramsOnAuction[i].auctionId
          );
        } else {
          endAuctionWithBid(
            _tokenAddress,
    ...
    for (
      uint256 i = initialEmogramsorder.length;
      i > initialEmogramsorder.length - 3;
      i--
    ) {
